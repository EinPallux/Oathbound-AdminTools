...
| **Ground** | Paint the terrain's ground surface from a big palette: **Grass, Forest floor, Jungle, Savanna, Desert sand, Beach sand, Dirt, Mud, Marsh, Mesa (red rock), Mountains (rocky), Rocky/snow peaks, Tundra, Ice, City (paved stone), Cobblestone road, Scorched/volcanic, Ash, Basalt, Corrupted, Town green**. **City** and **cobblestone** render as real paved stone — small repeat-tiled cobbles laid as a texture overlay, so stone size stays crisp regardless of terrain resolution — not grass. **Mountains** is a rocky grey-brown that lightens with height and snow-caps on the peaks. Every ground type now renders with a little **material grain** — grassy speckle + meadow patches, craggy stone mottle, fine sandy/snowy grit — so it reads as textured ground rather than one flat colour (and matches the game 1:1). Indices 0–6 are the gameplay biomes; the rest are cosmetic ground colours. (Was the "Biome" tool.) |
