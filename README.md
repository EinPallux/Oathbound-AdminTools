...
| **Flatten** | Slowly eases terrain toward a target, a little per pass (for restoring sculpted areas when undo is gone). Two modes: **To base level** pulls toward a fixed level (0 m = original flat ground), raising sunken / lowering raised ground back to flat; **Normalize** levels the spikes/roughness of an area toward its *own average height*, so it flattens a region **without lowering its base elevation** (e.g. smooth a jagged mountain top into a plateau). Raise *Strength* to go faster. |
| **Ground** | Paint the terrain's ground surface from a big palette: **Grass, Forest floor, Jungle, Savanna, Desert sand, Beach sand, Dirt, Mud, Marsh, Mesa (red rock), Mountains (rocky), Rocky/snow peaks, Tundra, Ice, City (paved stone), Cobblestone road, Scorched/volcanic, Ash, Basalt, Corrupted, Town green**. **City** and **cobblestone** render as real paved stone — small repeat-tiled cobbles laid as a texture overlay, so stone size stays crisp regardless of terrain resolution — not grass. **Mountains** is a rocky grey-brown that lightens with height and snow-caps on the peaks. In the Cube-World preview every cube top is laid with a **material texture** — grassy blades, craggy stone, or fine sandy/snowy grit — the same way City/Cobblestone read as cobbled stone, so each cube looks like its surface instead of a flat colour (and matches the game 1:1). Indices 0–6 are the gameplay biomes; the rest are cosmetic ground colours. (Was the "Biome" tool.) |
| **Water** | **Paint** water onto the terrain (like the Ground brush) at a chosen **surface height (Y)**. Water only shows where the level sits *above* the ground, so it fills lakes, basins and gorges up to that height — sculpt/lower the ground first for depth, set a different Y to pool water at another elevation, and tick **Erase** to remove it. (Replaces the old press-drag Lake tool; legacy circular lakes still load.) |
| **River / Road** | Click to drop polyline points, double-click or Enter to finish. Draped over the terrain. Roads have a **Surface** picker — **City road** (cobbled stone), **Grassland road** (packed dirt) or **Sandland road** (desert sand) — a realistic repeat-tiled texture that reads right for its environment, in the editor preview and 1:1 in-game. |
| **Assets** | Place props (single or scattered) from a 110+-asset library — every built-in Oathbound prop, a **pre-made set** of village buildings, grand **medieval city buildings** (cathedral, castle keep, town hall, guildhall, city manor, gatehouse, mage/bell/citadel towers, market hall, …), walls / towers / bridges / town props / ruins / extra trees & nature, plus your own custom assets. Each shown as a **live 3D thumbnail**. A **Height offset** slider raises/lowers what you place (e.g. a bridge above a lake). |
...
**Terrain: Smooth ⇄ Cubic** (top bar): preview the terrain as fine stepped cubes — a **Cube World / Trove** look — while you author, with the City/Cobblestone stone texture laid on the cube tops. The cubes are the **same size as in-game** (3 m): the preview draws a **fine-cube bubble around wherever the camera is looking** and re-centres as you pan (a huge map can't be voxelised whole at that size — the game solves it the same way, a cube bubble around the player), so the preview reads 1:1 with the player's view instead of the old giant whole-map blocks. In the editor this is **preview-only**: sculpting, painting and the exported `oathbound-map.json` are unchanged (the smooth terrain is hidden behind the cubic preview). Toggle back to **Smooth** for a full-map overview or when the flat surface is easier to edit on. The **game always renders in Cube World style** (fine cube bubble around the player, real cube collision) — there's no in-game switch; this editor toggle is just for authoring.
