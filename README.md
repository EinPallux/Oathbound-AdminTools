...
| **Water** | **Paint** water onto the terrain (like the Biome brush) at a chosen **surface height (Y)**. Water only shows where the level sits *above* the ground, so it fills lakes, basins and gorges up to that height — sculpt/lower the ground first for depth, set a different Y to pool water at another elevation, and tick **Erase** to remove it. (Replaces the old press-drag Lake tool; legacy circular lakes still load.) |
